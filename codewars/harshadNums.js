let Harshad = function() {
  ...
  return {
    isValid: function( number ) {
      return number % number.toString().split('').map(dig => parseInt(dig)).reduce((a, b) => a + b) === 0
    },
    getNext: function( number ) {
      let nextNum = number + 1
      return this.isValid(nextNum) ? nextNum : this.getNext(nextNum)
    },
    getSerie: function( count, start ) {
      let serie = []
      start === undefined ? start = 0 : start = start
      while(serie.length < count) {
        serie.push(this.getNext(serie[serie.length - 1] || start))
      }
      return serie
    }
  }
...
console.log(Harshad().isValid(27))
console.log(Harshad().getNext(36))
console.log(Harshad().getSerie(3, 36))
